...
internal final class YAMLHandler {
    ...
    static func handlerWithWorkSpace(_ workspace: DirectoryPath, template: TemplateType) -> YAMLHandler {
    ...
    private init(currentDirectory: DirectoryPath = .current, template: TemplateType) {
        ...
        self.configFilePath = currentDirectory.appendFileName(template.name, ext: "yml")
